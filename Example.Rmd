...
base_attachment_url <- 'https://services3.arcgis.com/9nfxWATFamVUTTGb/arcgis/rest/services/service_12179aa8762647dba17ff539184807a7/FeatureServer/0/queryAttachments'
...
for (row_number in 1:nrow(dataset)) {
  ...
  if (!is.null(parsed_attachment_json$attachmentGroups$attachmentInfos[[1]])) {
    for (attachment_row_number in 1:nrow(parsed_attachment_json$attachmentGroups$attachmentInfos[[1]])) {
      attachment_row <- parsed_attachment_json$attachmentGroups$attachmentInfos[[1]] %>% slice(attachment_row_number)
